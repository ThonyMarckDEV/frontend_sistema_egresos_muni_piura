...
const ModalInfoProveedor = ({ isOpen, onClose, proveedorId }) => {
  // 3. Estado adaptado
  ...
  useEffect(() => {
    ...
    if (isOpen && proveedorId) {
      const fetchData = async () => {
        setLoading(true);
        setError(null);
        setProveedorData(null); // Limpia datos anteriores
        try {
          // 6. Llamar al servicio correcto
          const response = await getProveedorById(proveedorId);
          
          // Asumo que el servicio devuelve el objeto proveedor (o response.data)
          ...
          setProveedorData(data); // 7. Guardar datos
  ...
  }, [isOpen, proveedorId]); // 9. Dependencia actualizada
    ...
    >
      ...
      >
        ...
        <div>
          ...
          {/* 11. Usar proveedorData */}
          {proveedorData && (
            <div className="max-h-[70vh] overflow-y-auto pr-2">
              <h4 className="text-lg font-medium mb-3 text-gray-700">Datos del Proveedor</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4">
                {/* Campos adaptados para Proveedor */}
                <InfoField label="Nombre / Razón Social" value={proveedorData.nombre} />
                <InfoField label="RUC" value={proveedorData.ruc} />
                <InfoField label="DNI" value={proveedorData.dni} />
                <InfoField 
                  label="Estado" 
                  value={proveedorData.estado === 1 ? 'Activo' : 'Inactivo'} 
                />
              </div>
              {/* Campo de descripción (ocupa 2 columnas) */}
              <div className="mt-4">
                <InfoField label="Descripción" value={proveedorData.descripcion} />
              </div>
            </div>
          )}
        </div>
...
export default ModalInfoProveedor;